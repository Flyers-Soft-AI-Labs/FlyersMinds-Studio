...
export function GenerationToolbar({
  ...
  // PDF handler — adds files to existing list, deduplicating by name
  const handleFilesSelect = (incoming: FileList | File[]) => {
    ...
    if (valid.length > 0) {
      onPdfError(null);
      onPdfFilesChange([...pdfFiles, ...valid]);
    }
  };
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      ...
      {/* ── PDF (parser + multi-upload) combined Popover ── */}
      <Popover>
          ...
          <div className="p-3 space-y-2">
                ...
                {pdfFiles.map((file, idx) => (
                  <div key={`${file.name}-${idx}`} className="flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-2.5 py-1.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-xs font-medium leading-none">{file.name}</p>
            ...
            {pdfFiles.length > 0 && (
              <button
                ...
                className="w-full text-left text-xs text-destructive/70 hover:text-destructive hover:underline"
              >
                Remove all PDFs
              </button>
            )}
