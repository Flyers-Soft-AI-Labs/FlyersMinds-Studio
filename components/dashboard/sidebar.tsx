...
export function DashboardSidebar({
  ...
  const itemBase = (active: boolean) =>
    cn(
      ...
      active
        ? 'border-white/25 bg-[linear-gradient(90deg,rgba(255, 255, 255,0.18),rgba(147,51,234,0.14))] text-white shadow-[0_0_28px_rgba(255, 255, 255,0.14)]'
  ...
  const iconWrap = (active: boolean) =>
    cn(
      ...
      active
        ...
        : 'border-white/[0.08] bg-white/[0.04] text-slate-400 group-hover:border-white/25 group-hover:bg-white/10 group-hover:text-white',
    );
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 w-72 border-r border-white/10 bg-[linear-gradient(180deg,rgba(2,6,23,0.97),rgba(3,7,18,0.94))] px-5 py-6 shadow-[0_20px_80px_rgba(0,0,0,0.45)] backdrop-blur-xl transition-transform duration-300 before:pointer-events-none before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_top_left,rgba(255, 255, 255,0.16),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(168,85,247,0.12),transparent_24%)] lg:translate-x-0',
        ...
        <div className="relative flex h-full flex-col">
          ...
          <div className="mb-8 flex items-center justify-between">
            <Link href="/" className="group flex items-center gap-3" onClick={onClose}>
              <div className="flex size-11 items-center justify-center rounded-2xl border border-white/25 bg-[linear-gradient(180deg,rgba(255, 255, 255,0.22),rgba(255, 255, 255,0.08))] shadow-[0_0_32px_rgba(255, 255, 255,0.28)] transition-all duration-300 group-hover:shadow-[0_0_44px_rgba(255, 255, 255,0.38)]">
                <Sparkles className="size-5 text-white" />
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-[0.32em] text-white/70">OpenMAIC</p>
            ...
            <button
              type="button"
              className="flex size-10 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-slate-400 transition-all duration-300 hover:-translate-y-0.5 hover:border-white/25 hover:bg-white/10 hover:text-white lg:hidden"
              onClick={onClose}
            >
              <X className="size-5" />
            </button>
            ...
            {linkItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {active && <ChevronRight className="size-3.5 shrink-0 text-white/25" />}
                </Link>
              );
            })}
