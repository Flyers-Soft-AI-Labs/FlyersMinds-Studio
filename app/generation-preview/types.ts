...
export interface GenerationSessionState {
  ...
  // Multi-PDF support — one entry per attached PDF
  pdfEntries?: Array<{
    storageKey: string;
    fileName: string;
    providerId?: string;
  ...
  // Legacy single-PDF fields (kept for backward compatibility)
  ...
  // User-supplied classroom name (set before generation begins)
...
export const getActiveSteps = (session: GenerationSessionState | null) => {
  return ALL_STEPS.filter((step) => {
    if (step.id === 'pdf-analysis') {
      return (session?.pdfEntries?.length ?? 0) > 0 || !!session?.pdfStorageKey;
    }
